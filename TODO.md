...
- [x] **Long-audio transcription.** A single NeMo `transcribe` call truncated long audio (cut off ~32 s of a 46 s clip on real hardware 2026-09-13). Fixed: audio longer than `_CHUNK_SECONDS` (30 s) is transcribed in overlapping 30 s windows (5 s overlap) and stitched onto one timeline with gap-free, duplicate-free tiling (`_chunk_windows`), offsetting segment + word timestamps. ASR also logs a coverage warning if the transcript still ends >3 s before the audio does. Tiling/offset logic unit-tested; the NeMo call path itself still needs a real-hardware re-run to confirm the full 46 s is now covered.
